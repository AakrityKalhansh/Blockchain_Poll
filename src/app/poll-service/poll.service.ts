...
export class PollService {
  ...
  async getPolls(): Promise<Poll[]> {
    ...
    const totalPolls = this.web3.call('getTotalPolls');
    ...
    for (let i = 0; i < (await totalPolls); i++) {
  ...
  vote(pollId: number, voteNumber: number) {
    this.web3.executeTranscation('vote', pollId, voteNumber);
  }

  createPoll(poll: PollForm) {
    this.web3.executeTranscation(
      'createPoll',
      poll.question,
      poll.thumbnail || '',
